...
# پروژه VODKA (Project VODKA)

### قدرتمندترین دروازه بدون سرور روی Cloudflare Workers

**VODKA** (به معنای *پVODKA/مخفی*) یک پروکسی معکوس امن، سبک و قابل سفارشی‌سازی است که به طور کامل روی Edge اجرا می‌شود. این پروژه Worker کلودفلر شما را به یک دروازه قدرتمند و مبهم با پشتیبانی از پروتکل‌های **VLESS** و **Trojan** تبدیل می‌کند که از طریق یک داشبورد زیبا و خودکفا مدیریت می‌شود.
...
[![JavaScript](https://img.shields.io/badge/JavaScript-100%25-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)](https://github.com/itsyebekhe/VODKA/blob/main/_worker.js)
...
VODKA فقط یک اسکریپت پروکسی نیست — یک راه‌حل مدیریت کامل است که برای **مخفی‌کاری**، **سرعت** و **سادگی** طراحی شده.

- 🛡️ **پVODKA در دید عموم:** درخواست‌های غیرمجاز به طور خودکار به سایت‌های معتبر (مثل `ubuntu.com` یا `docker.com`) هدایت می‌شوند.
...
- **نصب‌کننده تحت وب:** [https://erpycode.github.io/VODKA-installer/](https://erpycode.github.io/VODKA-installer/)
...
2. نامی بگذارید (مثلاً `VODKA-core`) و Deploy کنید.
3. **Edit code** را باز کنید، کد placeholder را پاک کنید و محتوای کامل [`_worker.js`](https://github.com/itsyebekhe/VODKA/blob/main/_worker.js) را پیست کنید.
...
1. با کلید پیش‌فرض `رمز پیش‌فرضی که در Cloudflare Secret با نام `VODKA_DEFAULT_PASSWORD` تعیین می‌کنید` وارد شوید.
...
ربات پیشنهادی: [@VODKAproxyipbot](https://t.me/VODKAproxyipbot)

### پنل‌های لینک‌شده (Other Nodes)

اتصال پنل‌های VODKA دیگر با **URL + API Key** برای مدیریت متمرکز و انتشار بروزرسانی.  
...
| Relay IP | [@VODKAproxyipbot](https://t.me/VODKAproxyipbot) |
| نصب آسان (تلگرام) | [@itsyebekhebot](https://t.me/itsyebekhebot) |
| نصب‌کننده وب | [erpycode.github.io/VODKA-installer](https://erpycode.github.io/VODKA-installer/) |
...
[⭐ ستاره بدهید](https://github.com/itsyebekhe/VODKA) · [🐛 گزارش خطا](https://github.com/itsyebekhe/VODKA/issues) · [🌐 نسخه انگلیسی](./README.md)
...
## احراز هویت دو مرحله‌ای VODKA — بدون KV
این نسخه برای ورود به پنل هیچ KV لازم ندارد.

در Cloudflare Worker → Settings → Variables and Secrets دو Secret بساز:
- `VODKA_DEFAULT_PASSWORD` = رمز مرحله اول
- `VODKA_MASTER_PASSWORD` = رمز اصلی مرحله دوم

صفحه ورود ابتدا رمز پیش‌فرض را می‌گیرد و سپس رمز اصلی را دو بار می‌گیرد. رمزها داخل GitHub قرار نمی‌گیرند و در `wrangler.toml` هم نوشته نمی‌شوند.
