...
async function getAsset(env, path) {
    ...
    try {
        return await env.ASSETS.fetch(new Request(`https://assets.local${path}`));
    } catch (e) {
        return null;
    }
}
...
/*
 * VODKA Panel - Cloudflare Gateway
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            const routes = {
                ...
                setupStatus: `/${encodeURI(sysConfig.apiRoute)}/api/setup-status`,
                setup: `/${encodeURI(sysConfig.apiRoute)}/api/setup`,
            };
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    try {
                        const resp = await getAsset(env, '/dashboard.html');
                        if (!resp || !resp.ok) throw new Error('dashboard asset unavailable');
                        let html = await resp.text();
                        html = html.replace(/__CURRENT_VERSION__/g, CURRENT_VERSION); else {
                ...
                if (reqPath === routes.setupStatus) {
                    if (request.method !== "GET") return new Response("405", { status: 405 });
                    return await handleSetupStatus(request, env);
                }
                if (reqPath === routes.setup) {
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (isValidUser) {
                            try {
                                const resp = await getAsset(env, '/subscription.html');
                                if (!resp || !resp.ok) throw new Error('subscription asset unavailable');
                                let html = await resp.text();
...
// VODKA authentication without KV.
// Set these two values as Cloudflare Worker Secrets: 
// VODKA_DEFAULT_PASSWORD and VODKA_MASTER_PASSWORD.
// The master password is intentionally not stored in the repository or in KV.
function applyConfiguredMaster(env) {
    const master = String(env?.VODKA_MASTER_PASSWORD || "");
    if (master) sysConfig.masterKey = master;
    return master;
}
...
async function handleSetup(request, env) {
    try {
        const data = await request.json();
        const defaultPassword = String(data.defaultPassword || "");
        const masterPassword = String(data.masterPassword || "");
        const confirmPassword = String(data.confirmPassword || "");
        const expectedDefault = String(env?.VODKA_DEFAULT_PASSWORD || "");
        const expectedMaster = String(env?.VODKA_MASTER_PASSWORD || "");
        if (!expectedDefault || !expectedMaster) {
            return new Response(JSON.stringify({success:false,error:"VODKA_DEFAULT_PASSWORD و VODKA_MASTER_PASSWORD باید به عنوان Secret تنظیم شوند."}), {status:500, headers:{"Content-Type":"application/json"}});
        }
        if (defaultPassword !== expectedDefault) {
            return new Response(JSON.stringify({success:false,error:"رمز پیش‌فرض اشتباه است."}), {status:401, headers:{"Content-Type":"application/json"}});
        }
        if (masterPassword.length < 8) {
            return new Response(JSON.stringify({success:false,error:"رمز اصلی باید حداقل ۸ کاراکتر باشد."}), {status:400, headers:{"Content-Type":"application/json"}});
        }
        if (masterPassword !== confirmPassword) {
            return new Response(JSON.stringify({success:false,error:"دو رمز اصلی یکسان نیستند."}), {status:400, headers:{"Content-Type":"application/json"}});
        }
        if (masterPassword !== expectedMaster) {
            return new Response(JSON.stringify({success:false,error:"رمز اصلی با VODKA_MASTER_PASSWORD مطابقت ندارد."}), {status:401, headers:{"Content-Type":"application/json"}});
        }
        sysConfig.masterKey = expectedMaster;
        return new Response(JSON.stringify({success:true}), {headers:{"Content-Type":"application/json","Cache-Control":"no-store"}});
    } catch (e) {
        return new Response(JSON.stringify({success:false,error:"درخواست راه‌اندازی نامعتبر است."}), {status:400, headers:{"Content-Type":"application/json"}});
    }
}
